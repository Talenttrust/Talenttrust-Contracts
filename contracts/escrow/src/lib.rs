...
impl Escrow {
    ...
    ///
    /// Rejects [`EscrowError::SelfRating`] when `contract.client == contract.freelancer`.
    /// This is defense-in-depth alongside create-time [`EscrowError::InvalidParticipant`],
    /// covering degenerate contracts where roles may collapse (e.g. after client migration).
