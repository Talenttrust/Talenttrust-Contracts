...
fn clientandarbiter_matrix_required_approvals_or_logic() {
    ...
    let result = client.try_release_milestone(&id1, &client_addr, &0, &0);
    ...
    let result = client.try_release_milestone(&id2, &arbiter_addr, &0, &0);
...
fn multisig_matrix_allowed_release_callers() {
    ...
    let result = client.try_release_milestone(&id2, &freelancer_addr, &0, &0);
