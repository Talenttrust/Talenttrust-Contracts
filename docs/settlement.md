...
`AccountingInvariantViolated` (error 44). `accumulated_protocol_fees` is the
...
`refund_unreleased_milestones` (`src/refund_impl.rs`) performs the same
`available_balance >= refund_amount` check and additionally verifies the live
SAC token balance covers the transfer (`InsufficientEscrowBalance`).
...
- `Split(s)` → rejected (`InvalidDisputeSplit`) if either leg is negative or
  exceeds `available`
...
- Refund: all refunded → `Refunded`; all released-or-refunded (mixed) →
  `Completed`; otherwise stays `Funded` (`refund_impl.rs::update_contract_status`).
...
- `Cancelled` and `Refunded` are terminal for value movement: deposits and
  refunds are rejected (`ContractCancelled` / `ContractRefunded`).
- `finalize_contract` writes immutable close metadata; afterwards any
  contract-specific mutation fails with `AlreadyFinalized`.

### I8. State is finalized before token transfers

In `bind_settlement_token`, `deposit_funds`, and `release_milestone`, all
state checks and mutations are completed before (or atomically with) the
`token::Client::transfer` call. Because Stellar/Soroban executions are
atomic, a panicking transfer reverts the whole transaction — the books can
never record a movement that did not happen, and a malicious token contract
re-entering during transfer sees finalized state (see
`src/lib.rs::bind_settlement_token` docs).
...
| `raise_dispute` / `resolve_dispute` | Freezes releases; arbiter resolution pays out per I6 and sets the final status. Emits `dispute`/`resolved`. |
...
`get_settlement_token`, `is_settlement_token_bound`.
...
4. **Dispute on milestone 1.** With an arbiter assigned, `raise_dispute` moves
   ...
   (note: the 25_000 accrued fee remains reserved). The arbiter resolves
   `Split { client_amount: 1_000_000, freelancer_amount: 1_000_000 }`:
   `refunded_amount = 4_000_000`, `released_amount = 1_975_000`, final status
   `Completed` (since `refunded_amount != funded_amount`).
   Final books: `released 1_975_000 + refunded 4_000_000 + fees 25_000 = 6_000_000 = funded_amount` — fully conserved.
5. **Treasury withdrawal.** `withdraw_protocol_fees(25_000, treasury)` moves
   the accrued fee out; the contract token balance drops to zero.
